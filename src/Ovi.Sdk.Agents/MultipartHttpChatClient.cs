...
/// </list>
/// <para>For a working client today, use any existing <see cref="IChatClient"/> implementation —
/// e.g. OllamaSharp's client for a local Ollama server — anywhere the SDK accepts a chat client.</para>
/// </remarks>
public class MultipartHttpChatClient : IChatClient
{
    ...
    public MultipartHttpChatClient(Uri endpoint, HttpClient? httpClient = null, string? modelId = null)
    {
    ...
    protected virtual MultipartFormDataContent CreateRequestContent(IEnumerable<ChatMessage> messages, ChatOptions? options)
    {
        ...
        var serializedMessages = JsonSerializer.Serialize(
            messages.Select(message => new { role = message.Role.Value, text = message.Text }),
            OviJson.DefaultOptions);
